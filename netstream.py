...
class netstream:
    def __init__(self, fmt = 0):
        ...
        self.wbuf = b''
        self.rbuf = b''
        self.stat = 0 # 0 = close 1 = connecting 2 = connected
    ...
    def send(self, data):
        ...
        self.sendraw(wsize + data.encode('utf-8'))
        return 0
...
class nethost:
    def __init__(self, fmt = 0):
        ...
        if fmt:
            self.bfmt = '!H'
        self.timeout = 70.0
        self.timeslap = int(time.time() * 1000)
        self.period = 0
    ...
    def process(self):
        ...
        for pos in range(len(self.clients)):
            ...
            if (client.status() == 0) or (timeout >= self.timeout):
                ...
                self.queue.append((NET_LEAVE, hid, tag, b''))
        ...
        if period > 0:
            while self.timeslap < current:
                self.queue.append((NET_TIMER, 0, 0, b''))
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if event == NET_DATA:
            host.send(wparam, 'RE: ' + data.decode('utf-8'))
            if data == b'exit':
